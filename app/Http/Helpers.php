...
if (!function_exists('my_asset')) {
    ...
    function my_asset($path, $secure = null)
    {
        ...
        return app('url')->asset(asset_url_path($path), $secure);
    }
}

if (!function_exists('asset_url_path')) {
    function asset_url_path($path)
    {
        $prefix = trim(config('app.asset_url_prefix', 'public'), '/');
        $path = ltrim($path, '/');
...
if (!function_exists('custom_upload_file')) {
    function custom_upload_file($file)
    {
        ...
        try {
            ...
            $img->encode($extension, 75)->save(public_path($path));
