...
<div class="label-box">
        ...
        <table width="100%" cellpadding="0" cellspacing="0">
            <tr>
                <td style="width: 40%; vertical-align: middle;">
                    @if($logo)
                        <img src="{{ uploaded_asset($logo) }}" class="logo"  style="display:inline-block;" height="20">
                    @else
                        <img src="{{ static_asset('assets/img/placeholder.jpg') }}" class="logo"  style="display:inline-block;" height="20">
                    @endif
                </td>
    ...
    <div class="barcode-strip">
        @if($btype === 'qrcode')
            {!! str_replace($removedXML, '', QrCode::size(30)->generate($bval)) !!}
        @else
            {!! $makeSvg($bval, $btype) !!}
        @endif
        @if($showTxt)
    ...
    <div class="footer">
        <div class="footer-left">
            <div>{{ translate('Date') }}: {{ date('d/m/y', $order->date) }} {{ date('h:ia', $order->date) }}</div>
        </div>
    </div>
