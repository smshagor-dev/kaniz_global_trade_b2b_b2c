        ...
        <div class="col-lg-10 mx-auto">
            ...
            <div class="card">
                <div class="card-body p-0">
                        ...
                        @foreach (get_all_active_language() as $key => $language)
                            <li class="nav-item">
                                <a class="nav-link text-reset @if($language->code == $lang) active @endif py-3"
                                    href="{{ route('flash_deals.edit', ['id' => encrypt($flash_deal->id), 'lang' => $language->code]) }}">
                    ...
                    <div class="card-body">
                        <form action="{{ route('flash_deals.update', $flash_deal->id) }}" method="POST">
                            @csrf
                            ...
                            <div class="row">
                                <div class="col-12 col-sm-12 col-md-6 col-lg-6">
                                    <div class="form-group row">
                                        ...
                                        <div class="col-12">
                                            <input type="text" placeholder="{{translate('Title')}}" name="title"
                                                class="form-control"
                                                value="{{ $flash_deal->getTranslation('title', $lang) }}" required>
                                        </div>
                                    </div>
                                    ...
                                    @endphp
                                    <div class="form-group row">
                                        <label class="col-12 control-label">{{translate('Date')}}</label>
                                        <div class="col-12">
                                ...
                                <div class="col-12 col-sm-12 col-md-6 col-lg-6">
                                    <div class="form-group row">
                                        <label class="col-12 control-label">{{translate('Banner')}}</label>
                                        <div class="col-12">
                                                ...
                                                <div class="input-group file-upload-input border border-dashed border-gray-400 rounded-1 w-120px h-120px d-flex align-items-center justify-content-center"
                                                    ...
                                                    <div
                                                        ...
                                                        <img src="{{ static_asset('assets/img/plus-lg.svg') }}"
                                                            class="w-40px h-40px w-md-64px h-md-64px" alt="banner">
                                                    </div>
                                                    <input type="hidden" name="banner" class="selected-files"
                                                        value="{{ $flash_deal->banner }}">
                                                </div>
...
@section('script')
    <script type="text/javascript">
        ...
        const flashDealId = {{ $flash_deal->id }};

        let addedProductIds = new Set([
            @foreach($flash_deal->flash_deal_products as $fdp)
                '{{ $fdp->product_id }}',
            @endforeach
                            ]);
        ...
        $(document).ready(function () {
            ...
            if (existingIds.length > 0) {
                $.post(
                    '{{ route('flash_deals.product_discount_edit') }}',
                    {
                        _token: '{{ csrf_token() }}',
                ...
                $.post(
                    ...
                    function (data) {
                        ...
                        addedProductIds.forEach(function (id) {
                            $('.flash-deal-product-check[data-product-id="' + id + '"]')
                            .prop('checked', true)
                            .prop('disabled', true);
                        });
                    }
        ...
        function showSaveMsg(productId, text, hideAfter = 0) {
            ...
            $msg.show();
            if (hideAfter > 0) setTimeout(function () { $msg.hide(); }, hideAfter);
        }
        ...
        $(document).on('change', '.discount-type-select', function () {
            ...
            const type      = $(this).val();
            const unitPrice = parseFloat($('#discount-input-' + productId).data('unit-price'));
