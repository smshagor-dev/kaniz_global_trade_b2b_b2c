<form action="{{ route('your_currency.update') }}" method="POST" >
    ...
    <div class="modal-body">
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-10">
                <input type="number" lang="en" step="0.00000001" min="0" placeholder="{{translate('Exchange Rate')}}" id="exchange_rate" name="exchange_rate" value="{{ $currency->exchange_rate }}" class="form-control" required>
            </div>
        </div>
        <div class="form-group row">
            <label class="col-sm-2 col-from-label" for="decimal_places">{{translate('Decimals')}}</label>
            <div class="col-sm-10">
                <input type="number" min="0" max="6" id="decimal_places" name="decimal_places" value="{{ $currency->decimal_places ?? 2 }}" class="form-control" required>
            </div>
        </div>
        <div class="form-group row">
            ...
            <div class="col-sm-10">
                <select class="form-control aiz-selectpicker" id="symbol_position" name="symbol_position">
                    <option value="prefix" @selected(($currency->symbol_position ?? 'prefix') === 'prefix')>{{translate('Prefix')}}</option>
                    <option value="prefix_spaced" @selected(($currency->symbol_position ?? 'prefix') === 'prefix_spaced')>{{translate('Prefix with space')}}</option>
                    <option value="suffix" @selected(($currency->symbol_position ?? 'prefix') === 'suffix')>{{translate('Suffix')}}</option>
                    <option value="suffix_spaced" @selected(($currency->symbol_position ?? 'prefix') === 'suffix_spaced')>{{translate('Suffix with space')}}</option>
                </select>
            </div>
        </div>
    </div>
