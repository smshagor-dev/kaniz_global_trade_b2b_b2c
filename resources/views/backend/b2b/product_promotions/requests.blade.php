...
<div class="aiz-titlebar mt-2 mb-3">
    <div class="row align-items-center">
        <div class="col-md-6">
            <h3 class="h3">{{ translate('Sponsored Product Package Requests') }}</h3>
        </div>
    </div>
</div>
        ...
        <div class="table-responsive">
            <table class="table aiz-table mb-0">
                ...
                <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-right">
                                @if ($request->status === 'pending')
                                    <form action="{{ route('admin.b2b.product-promotions.requests.approve', $request->id) }}" method="POST" class="d-inline-block">
                                        @csrf
                                        <button type="submit" class="btn btn-soft-success btn-sm">{{ translate('Approve') }}</button>
                                    </form>
                                    <form action="{{ route('admin.b2b.product-promotions.requests.reject', $request->id) }}" method="POST" class="d-inline-block">
                                        @csrf
                                        <input type="hidden" name="rejection_note" value="{{ translate('Sponsored product package request rejected by admin.') }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="text-center">{{ translate('No sponsored product package requests found') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
