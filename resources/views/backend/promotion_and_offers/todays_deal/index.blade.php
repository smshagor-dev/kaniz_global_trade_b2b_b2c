...
@section('content')
    ...
    <div class="aiz-titlebar text-left pb-5px">
        <div class="row align-items-center">
            <div class="col-auto">
                <h1 class="h3 fw-bold">{{ translate('Todays Deal Products') }}</h1>
            </div>
    ...
    <div class="card">
        ...
        <!--Nav Tab -->
         <div
            ...
            <div class="">
                @if (auth()->user()->can('add_todays_deal_products'))
                    <a href="javascript:void(0);" onclick="openRightcanvas()" class="position-relative overflow-hidden add-new-btn">
                        <span class="position-relative z-2 pr-15px fs-14 fw-500 text-blue label-text">{{ translate('Todays Deal Products') }}</span>
                    ...
                    <div class="dropdown mb-2 mb-md-0 bg-light mt-2 mt-md-0 px-md-1 rounded-1">
                        ...
                        <div class="dropdown-menu dropdown-menu-right">
                            ...
                            <a class="dropdown-item confirm-alert text-danger fs-14 fw-500 hov-bg-light hov-text-blue"
                                ...
                                {{ translate('Remove From Todays Deal') }}</a>
                            @endcan
                        </div>
                    </div>
...
@section('script')
    <script type="text/javascript">
        ...
        function updateProductInTodaysDeal() {
            let allProductIds = [];
            ...
            const url = "{{ route('todays_deal_products.update') }}";
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        AIZ.plugins.notify('success', '{{ translate("Todays Deal products added successfully") }}');
                    } else {
        ...
        function single_remove(productId) {
            $.ajax({
                url: "{{ route('todays_deal_products.update') }}",
                method: 'POST',
                data: {
                    _token: AIZ.data.csrf,
                    all_ids: [productId],
                ...
                success: function(response) {
                    if (response.success) {
                        AIZ.plugins.notify('danger', '{{ translate("Product removed from Todays Deal successfully") }}');
        ...
        function bulk_remove() {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) { 
                        AIZ.plugins.notify('danger', '{{ translate("Selected products removed from Todays Deal") }}');
                        hideBulkActionModal(); 
                        getProducts(currentTab);
                    } else {
                        AIZ.plugins.notify('danger', response.message || '{{ translate("Operation failed") }}');
                    }
                },
                error: function() {
                    AIZ.plugins.invoke('danger', '{{ translate("Something went wrong") }}');
                }
            });
        }
        ...
        function singleDelete(productId) {
            ...
            $('#confirmation-question').text('{{ translate('Are you sure you want to remove this product from Todays Deal?') }}');
            $('#impact-message').text('{{ translate('Proceeding will remove this item from active Todays Deal listings. You cannot undo this action.') }}');
        ...
        function bulkDelete() {
            ...
            $('#confirmation-question').text('{{ translate('Are you sure you want to remove selected products from Todays Deal?') }}');
            $('#impact-message').text('{{ translate('Proceeding will remove selected items from active Todays Deal listings. You cannot undo this action.') }}');
        ...
        // Right Offcanvas JS Start
            ...
            function openRightcanvas() {
                ...
                document.body.classList.add('body-no-scroll');
                 $('#rightOffcanvas .action-btn').text("{{ translate('Add') }}").attr('onclick', 'updateProductInTodaysDeal()');
            }
            // Close function
