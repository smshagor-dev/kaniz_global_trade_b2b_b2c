...
<a href="javascript:void(0)" class="d-flex align-items-center @if (get_setting('header_element') != 6) px-3 @endif h-100" data-toggle="dropdown" data-display="static"
    ...
    @if (!in_array(get_setting('header_element'), [6, 10]))
        <span class="mr-2">
    ...
    @else  
        <span class="mr-1">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                class="{{ get_setting('header_element') !=6 ? 'top-text-color-visibility' : 'bottom-text-color-visibility' }}"
                style="@if(get_setting('header_element') == 10) color: {{ get_setting('top_header_text_color') }}; @else color: {{ get_setting('bottom_header_text_color') }}; @endif"
                ...
                @if(get_setting('header_element') != 10)
                    <rect x="9" y="10" width="6" height="2" fill="#ffffff" rx="0.3"/>
                @endif
            </svg>
        </span>
        <span class="d-none d-xl-block ml-1 fs-14 fw-700 {{ get_setting('header_element') !=6 ? 'top-text-color-visibility' : 'bottom-text-color-visibility' }}"
            style="@if(get_setting('header_element') == 10) color: {{ get_setting('top_header_text_color') }}; @else color: {{ get_setting('bottom_header_text_color') }}; @endif">{{translate('Cart')}}</span>
        <span class="nav-box-text d-none d-xl-block ml-1 fw-700 {{ get_setting('header_element') !=6 ? 'top-text-color-visibility' : 'bottom-text-color-visibility' }}"
            style="@if(get_setting('header_element') == 10) color: {{ get_setting('top_header_text_color') }}; @else color: {{ get_setting('bottom_header_text_color') }}; @endif">
...
<!-- Cart Items -->
<div class="dropdown-menu dropdown-menu-right dropdown-menu-lg p-0 stop-propagation rounded-0 mt-cart-dropdown-menu">
