...
@section('script')
    <script type="text/javascript">
        const hasOfflineCustomerPackageRoute = @json(Route::has('offline_customer_package_purchase_modal'));
        ...
        function payment_type(type) {
            ...
            if (type == 'online') {
                ...
                $("#select_type_cancel").click();
                if (!hasOfflineCustomerPackageRoute) {
                    AIZ.plugins.notify('warning', '{{ translate('Offline package payment is not available right now.') }}');
                    return;
                }

                $.post('{{ Route::has('offline_customer_package_purchase_modal') ? route('offline_customer_package_purchase_modal') : '' }}', {
                    _token: '{{ csrf_token() }}',
