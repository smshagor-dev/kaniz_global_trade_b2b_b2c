                                    ...
                                    <div>
                                        <p class="mb-1 text-truncate-2">
                                            ...
                                            @elseif($notification->type == 'App\Notifications\OrderTrackingNotification')
                                                @php
                                                    $orderCode  = $notification->data['order_code'];
                                                    $trackingCode  = $notification->data['tracking_code'];
                                                        $route = route('purchase_history.details', encrypt($notification->data['order_id']));
                                                        $route2 = route('orders.track', ['tracking_code' => $notification->data['tracking_code']]);
                                                        $orderCode = "<a href='".$route."'>".$orderCode."</a>";
                                                        $trackingCode = "<a href='".$route2."'>".$trackingCode."</a>";
                                                    $notifyContent = str_replace('[[order_code]]', $orderCode, $notifyContent);
                                                    $notifyContent = str_replace('[[tracking_code]]', $trackingCode, $notifyContent);
                                                @endphp
