                        ...
                        <tr>
                            ...
                            <td class="text-right pr-0">
                                @if ($wallet->offline_payment)
                                    @if ($wallet->approval && $wallet->added_by == 'admin')
                                        <div class="d-inline-block dropdown">
                                            <button type="button"
                                                class="btn btn-success btn-sm dropdown-toggle text-white px-3 py-1 rounded-1 w-165px"
                                                data-toggle="dropdown">
                                                {{ translate('Recharged By Admin') }}
                                            </button>
                                            <div class="dropdown-menu dropdown-menu-right ">
                                                <a class="dropdown-item text-secondary dropdown-bg-hover" href="javascript:void(0)" onclick="showPaymentReceipt({{ $wallet->id }})" title="{{ translate('Open Receipt') }}"><i class="las la-eye mr-2"></i>{{ translate('Open Receipt') }}</a>
                                            </div>
                                        </div>
                                    @elseif($wallet->approval)
                                        <div class="d-inline-block dropdown">
                                            <button type="button"
                                                class="btn btn-success btn-sm text-white px-3 py-1 rounded-0 w-165px"
                                                data-toggle="dropdown">
                                                {{ translate('Approved') }}
                                            </button>
                                        </div>
                                    @else
                                        <span class="badge badge-inline badge-primary p-3 fs-12" style="border-radius: 25px; min-width: 80px !important;">{{ translate('Pending') }}</span>
                                    @endif
                                @else
                                    N/A
                                @endif
