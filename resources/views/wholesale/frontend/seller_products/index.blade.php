...
<div class="row gutters-10 justify-content-center">
    ...
    <div class="col-md-4 mx-auto mb-3">
        <a href="{{ route('seller.b2b.product-promotions.index') }}" class="text-center bg-white shadow-sm hov-shadow-lg text-center d-block p-3 rounded h-100">
            <i class="las la-bullhorn mb-2 la-3x text-warning"></i>
            <span class="d-block sub-title mb-2">
                {{ translate('Sponsored Package') }}:
                {{ $currentSponsoredPackage?->name ?: translate('No Active Package') }}
            </span>
            <div class="text-muted mb-2">
                {{ translate('Remaining Promote Slots') }}:
                {{ is_null($remainingSponsoredSlots) ? translate('Unlimited') : $remainingSponsoredSlots }}
            </div>
            <div class="btn btn-outline-warning py-1">{{ translate('Manage Sponsored Products') }}</div>
        </a>
    </div>
            ...
            <table class="table aiz-table mb-0">
                <thead>
                    <tr>
                        ...
                        <th data-breakpoints="lg">{{translate('Sponsored')}}</th>
                    ...
                    <tr>
                        ...
                        <td>
                            <label class="aiz-switch aiz-switch-warning mb-0">
                                <input onchange="update_sponsored(this)" value="{{ $product->id }}" type="checkbox" @checked(in_array($product->id, $promotedProductIds ?? []))>
                                <span class="slider round"></span>
                            </label>
                        </td>
                        <td class="text-right">
...
@section('script')
    <script type="text/javascript">
        ...
        function update_sponsored(el){
            var status = el.checked ? 1 : 0;
            $.post('{{ route('seller.b2b.product-promotions.toggle-product', ['productId' => '__PRODUCT__']) }}'.replace('__PRODUCT__', el.value), {
                _token:'{{ csrf_token() }}',
                status:status
            }, function(response){
