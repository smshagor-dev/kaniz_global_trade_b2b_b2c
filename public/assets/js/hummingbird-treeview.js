(function($){
	...
	$.each(all_converter,function(e){
	    ...
	    var radio_name = converter.attr("data-radio-name");
		...
		if (numHyphen < numHyphen_next) {
		    ...
		    item = item + '<i class="las la-plus"></i>' + "\n";
		    item = item + '<label ' + data_str  + '>' + "\n";
		    if (boldParents){
			item = item + '<input id="' + id_str  + '" value="'+ value +'" data-id="' + data_id + '" type="checkbox" name="'+ chk_name +'" onChange="check_filter(event)"/> <b>' + treeText + '</b> ';
		    } else {
			item = item + '<input id="' + id_str  + '" value="'+ value +'" data-id="' + data_id + '" type="checkbox" name="'+ chk_name +'" onChange="check_filter(event)"/> ' + treeText;
		    }
		    item = item + '</label><input value="'+ value +'" type="radio" name="'+ radio_name +'" onChange="check_filter_main(event)"/>' + "\n";
		    item = item + '<ul>' + "\n";
		    //console.log(item);
		}
		...
		if (numHyphen == numHyphen_next) {
		    item = item + '<li>' +"\n";
		    item = item + '<label ' + data_str  + '>' + "\n";
		    item = item + '<input class="hummingbird-end-node" id="' + id_str + '" value="'+ value +'" data-id="' + data_id + '" type="checkbox" name="'+ chk_name +'" onChange="check_filter(event)"/> ' + treeText;
		    item = item + '</label><input value="'+ value +'" type="radio" name="'+ radio_name +'" onChange="check_filter_main(event)"/>' + "\n";
		    item = item + '</li>' + "\n";
		    //console.log(item);
		}		
	...
	$.each(anchor_vals, function(i,e){
		...
		if (pos[i] == "before") {
		    that_node.before('<li><label><input class="'+ Xclass  +'" id="'+ the_id[i]  +'" data-id="'+ data_id[i]  +'" type="checkbox" name="'+ chk_name +'"> '+ text[i]  +'</label><input class="'+ Xclass  +'" type="radio" name="'+ radio_name +'"></li>')
		}
		if (pos[i] == "after") {
		    that_node.after('<li><label><input class="'+ Xclass  +'" id="'+ the_id[i]  +'" data-id="'+ data_id[i]  +'" type="checkbox" name="'+ chk_name +'"> '+ text[i]  +'</label><input class="'+ Xclass  +'" type="radio" name="'+ radio_name +'"></li>')
		}
	    } else {
		var Xclass = "";
		//create subtree
		...
		$.each(children[i], function(f,g){
		    subtree = subtree + '<li><label><input class="'+ 'hummingbird-end-node'  +'" id="'+ g.id  +'" data-id="'+ g.data_id  +'" type="checkbox" name="'+ chk_name +'"> '+ g.text  +'</label><input class="'+ Xclass  +'" type="radio" name="'+ radio_name +'"></li>';
		});
		if (pos[i] == "before") {
		    that_node.before('<li>'+"\n"+'<i class="las '+ collapsedSymbol  +'"></i>'+ "\n" +'<label>'+"\n"+'<input class="'+ Xclass  +'" id="'+ the_id[i]  +'" data-id="'+ data_id[i]  +'" type="checkbox" name="'+ chk_name +'"> '+ boldParents_str1 + text[i] + boldParents_str2  +'</label><input class="'+ Xclass  +'" type="radio" name="'+ radio_name +'">'+ "\n" +'<ul>'+ "\n" + subtree  +'</ul>'+"\n"+'</li>');
		}
		if (pos[i] == "after") {
		    that_node.after('<li>'+"\n"+'<i class="las '+ collapsedSymbol  +'"></i>'+ "\n" +'<label>'+"\n"+'<input class="'+ Xclass  +'" id="'+ the_id[i]  +'" data-id="'+ data_id[i]  +'" type="checkbox" name="'+ chk_name +'"> '+ boldParents_str1 + text[i] + boldParents_str2  +'</label><input class="'+ Xclass  +'" type="radio" name="'+ radio_name +'">'+ "\n" +'<ul>'+ "\n" + subtree  +'</ul>'+"\n"+'</li>');
		}
...
// $clicked.closest('li').find('ul label input[type="checkbox"]').prop('checked', isChecked);
function check_filter(e) {
    e.preventDefault();

    const $clicked = $(e.target);
    const isChecked = $clicked.is(':checked'); 


    let $currentUl = $clicked.closest('ul');
    ...
    while ($currentUl.length) {
        ...
        if ($parentCheckbox.length) {
            if (isChecked) {
				...
				$parentCheckbox.attr('onclick', 'cursor_not_allowed(event)');
				$parentCheckbox.css('cursor', 'not-allowed'); 
				$parentCheckbox.closest('label').css('cursor', 'not-allowed');
            } else {
                const allSiblingsUnchecked = $currentUl.find('> li > label > input[type="checkbox"]:checked').length === 0;
                if (allSiblingsUnchecked) {
                    $parentCheckbox.prop('checked', false);
					$parentCheckbox.removeAttr('onclick');     
					$parentCheckbox.css('cursor', 'pointer');
					$parentCheckbox.closest('label').css('cursor', 'pointer');
                }
            }
        }
...
function check_filter_main(e) {
	...
	$('#treeview').find('input[type="checkbox"]').each(function() {
		...
		$(this).closest('label').css('cursor', 'pointer');
	});

    const $clicked = $(e.target);

   
    const $prevLabel = $clicked.prev('label');

   
    const $inputInside = $prevLabel.find('input');

   
    $inputInside.prop('checked', true);
	$inputInside.attr('onclick', 'cursor_not_allowed(event)'); 
	$inputInside.css('cursor', 'not-allowed');
	$inputInside.closest('label').css('cursor', 'not-allowed');

	$inputInside.trigger('change');
}



function cursor_not_allowed(e) {
