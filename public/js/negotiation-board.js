    ...
    function initBoard(root) {
        ...
        function renderList() {
            var items = state.listItems.filter(function (item) {
                ...
                return [item.title, item.subtitle, item.latest_message, item.status].join(' ').toLowerCase().indexOf(state.searchTerm) !== -1;
            });
        ...
        function loadThread(id, options) {
            ...
            return fetchJson(threadUrl(id)).then(function (payload) {
                renderThread(payload.data || {}, { wasNearBottom: settings.silent ? wasNearBottom : true });
        ...
        function refreshThreadSilently() {
            if (state.activeId) {
